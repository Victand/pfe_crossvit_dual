    ...
    def __init__(self, dim, patches, depth, num_heads, mlp_ratio, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        for d in range(num_branches):
            ...
            for i in range(depth[d]):
                ...
                tmp.append(
                        RecordingBlock(dim=dim[d], num_heads=num_heads[d], mlp_ratio=mlp_ratio[d], 
...
class DualCrossVit(crossvit.VisionTransformer):
    ...
    def __init__(self, img_size=(224, 224), patch_size=(8, 16), in_chans=3, num_classes=2, embed_dim=(192, 384), depth=([1, 3, 1], [1, 3, 1], [1, 3, 1]),
        ...
        super().__init__(img_size, patch_size, in_chans, num_classes, embed_dim, depth, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, drop_path_rate, hybrid_backbone, norm_layer, multi_conv, **kwargs)
        self.kwargs = kwargs
    ...
    def forward_features(self, x_small, x_large, weights):
        ...
        for i in range(self.num_branches):
            ...
            if current_img.shape[-2] != expected_size or current_img.shape[-1] != expected_size:
                x_ = torch.nn.functional.interpolate(current_img, size=(expected_size, expected_size), mode='bicubic',
                                                     align_corners=False)
            else:
                x_ = current_img
            ...
            if weights is not None:
                ...
                if weights.shape[1] != tmp.shape[1]:
                    ...
                    w_resized = torch.nn.functional.interpolate(w_2d, size=(target_grid_size, target_grid_size),
                                                                mode='nearest')
            ...
            if tmp.shape[1] != pos_embed.shape[1]:
                ...
                pos_embed_patches = torch.nn.functional.interpolate(pos_embed_patches, size=tmp.shape[1] - 1,
