...
class DualCrossVit(crossvit.VisionTransformer):
    ...
    def __init__(self, img_size=(224, 224), patch_size=(8, 16), in_chans=3, num_classes=2, embed_dim=(192, 384),
        ...
        super().__init__(img_size, patch_size, in_chans, num_classes, embed_dim, depth, num_heads, mlp_ratio, qkv_bias,
                         qk_scale, drop_rate, attn_drop_rate, drop_path_rate, hybrid_backbone, norm_layer, multi_conv,
                         **kwargs)
        self.kwargs = kwargs
    ...
    def forward_features(self, x_small, x_large, weights, num_patches):
        ...
        if weights is not None:
            ...
            if weights.shape[1] != tmp_l.shape[1]:
                ...
                weights = torch.nn.functional.interpolate(w_2d, size=(target_g, target_g), mode='nearest').view(B_large,
                                                                                                                -1, 1)
            tmp_l = tmp_l * weights
