...
from scripts.IoULoss import IoUConstrainedLoss
from scripts.utils.writersAndPlotters import *
from scripts.attentionRollout import get_trainable_heatmap, get_iou_training


def dual_eval(model, data_loader, loss_fn, device, patch_size, plot_pounderation=False, debug=False, failed_preds=0):
    """
    ...
    with torch.no_grad():
        ...
        for batch_id, (original_image, segmented_image, label_gt, weights) in enumerate(data_loader):
...
def train_one_epoch(model,epoch_idx, epochs, best_val_loss, data_loader_train, data_loader_val, device, loss_fn, optimizer, scheduler, patch_size, pounderation, save, output_dir,log_file, args=None):
    model.train()
    total_train_loss = 0
    ...
    avg_val_loss, accuracy, f1_score, avg_iou = dual_eval(model, data_loader_val, loss_fn, device, patch_size, plot_ponderation)

    if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            write_in_logs(log_file,message=f"\nBest val loss yet : {best_val_loss}")
            if save and (save=='BEST') and output_dir:
                torch.save(model.state_dict(), Path(output_dir) / f"best_model.pth")
    
    if save and (save=='LAST') and output_dir:
        torch.save(model.state_dict(), Path(output_dir) / f"last_model.pth")
...
def train(model, epochs, data_loader_train, data_loader_val, device, loss_fn, optimizer, scheduler, patch_size=16, pounderation=False, args=None, plot_loss=False, save=None, maskcaptor=None, output_dir='src\savedModels', log_file=None):
    """
    ...
    try:
        ...
        for epoch_idx in range(epochs):
            best_val_loss, avg_train_loss, avg_val_loss, accuracy, f1_score, avg_iou = train_one_epoch(model,epoch_idx, epochs,best_val_loss, data_loader_train, data_loader_val, device, loss_fn, optimizer, scheduler, patch_size, pounderation, save, output_dir, log_file, args)
    ...
    # Clean exit
        ...
        print(f"[Warning] Training interrupted by user.")
