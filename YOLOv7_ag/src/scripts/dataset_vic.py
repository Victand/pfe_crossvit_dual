...
class DualInputDataset(Dataset):
    def __init__(self, data_dir: str = DATA_PATH_STR, is_train: bool = True, image_size=(240, 240), patch_size=(16, 16),
                 ...
                 weight_function=linear_, use_yolo_weights=False, num_patches=16, patch_quotas=None):
    ...
    def __getitem__(self, idx):
        ...
        if len(final_list) > self.num_patches:
            ...
            padding = [torch.zeros((3, 224, 224)) for _ in range(self.num_patches - len(final_list))]
            final_list.extend(padding)

        selected_patches = [TF.normalize(p, self.mean, self.std) if p.max() > 0 else p for p in final_list]
...
def normalize_and_erase(dualdataset, img1, img2):
    ...
    if dualdataset.is_train and dualdataset.active_transforms["random_erase"] and random.random() < 0.1:
        i, j, h, w, v = transforms.RandomErasing.get_params(img1, scale=(0.02, 0.33), ratio=(0.3, 3.3), value=None)
        img1, img2 = TF.erase(img1, i, j, h, w, v), TF.erase(img2, i, j, h, w, v)
    return img1, img2

def prepare_dataloaders(train_ds, val_ds, batch_size, num_workers):
    t_ld = torch.utils.data.DataLoader(train_ds, batch_size=batch_size, num_workers=num_workers, shuffle=True)
    v_ld = torch.utils.data.DataLoader(val_ds, batch_size=int(2*batch_size), num_workers=num_workers, shuffle=False)
    return t_ld, v_ld
