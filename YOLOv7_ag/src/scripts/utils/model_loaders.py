...
def load_weights_for_symmetric_crossvit(symmetric_model:nn.Module,pretrained_model_name='crossvit_15_224'):
    """
...
def get_images_both_branches_as_tensors(img:Image,img_size_small:int,img_size_large:int, device, mean, std):
      """
      returns both images as tensors ready to be used by the model's forward method.
      """
      img_tensor_small = TF.normalize(TF.to_tensor(TF.center_crop(TF.resize(img, int(img_size_small*1.14)), img_size_small)), mean=mean, std=std)
      img_tensor_small = img_tensor_small.unsqueeze(0).to(device)
      img_tensor_large = TF.normalize(TF.to_tensor(TF.center_crop(TF.resize(img, int(img_size_large*1.14)), img_size_large)), mean=mean, std=std)
