...
def plot_weight_example(segmented_images_batch:tuple[Tensor,...], weights:Tensor, num_examples:int, patch_size:int, images_name=None, add_infos:str=""):
    """
...
def compare_functions_for_patches_ponderation(dataset, functions:dict, num_images:int=1):
        ...
        for sample in dataset.samples[:num_images]:
            ...
            segmented_image_tensor=TF.to_tensor(TF.center_crop(TF.resize(segmented_image, int(dataset.image_size[1]*1.14)), dataset.image_size[1]))
        ...
        for f_name,f in functions.items():
            ...
            plot_weight_example(imgs_and_names[0], weights, num_images, dataset.patch_size[1],imgs_and_names[1],f"Function : {f_name}\n")