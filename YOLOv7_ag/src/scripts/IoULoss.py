...
from scripts.attentionRollout import get_trainable_heatmap
...
class IoUConstrainedLoss(nn.Module):
    def __init__(self, model, captor, lambda_iou:float=0.1, temperature_mult:float=10.0, device='cuda'):
        super().__init__()
        self.model = model
        self.captor = captor
    ...
    def forward(self, logits, label_gt):
        ...
        target_mask = F.interpolate(target_mask, size=(self.model.img_size[0], self.model.img_size[0]), mode='nearest')
