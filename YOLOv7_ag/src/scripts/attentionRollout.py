...
def vector_to_heatmap(vector, branch_img_size:tuple[int,int], patch_size:tuple[int,int], target_size=(240, 240)):
    """
...
def perform_rollout(model, show_individuals=False):
    ...
    map_direct, map_indirect = vector_to_heatmap(direct_vector,(small_img_size,small_img_size),(small_patch_size,small_patch_size),target_size), vector_to_heatmap(indirect_vector,(large_img_size,large_img_size),(large_patch_size,large_patch_size),target_size)
...
def compute_iou(heatmap, segmented_image, img_size=240, threshold=0.5):
    ...
    img_tensor = TF.to_tensor(TF.center_crop(TF.resize(segmented_image, int(img_size*1.14)), img_size))
...
def get_iou_training(model,heatmap,segmented_image):
    ...
    target_mask = F.interpolate(target_mask, size=(model.img_size[0], model.img_size[0]), mode='nearest')
