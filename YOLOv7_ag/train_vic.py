...
def debug_full_diagnostic(x_small, img_large, weights, model, labels, classes, epoch, save_dir="saved/images"):
    ...
    if weights is not None:
        h_map = (weights[0] * STRATEGIE_ALPHAS[:weights.shape[1]].view(-1, 1, 1)).sum(dim=0).cpu().numpy()
    ...
    ax2.imshow(img_vis)
    ax2.imshow(vis, cmap='viridis', alpha=0.7, extent=(0, img_vis.shape[1], img_vis.shape[0], 0),
...
def train(model, train_loader, val_loader, epochs, lr, batch_size=16, save_dir="saved", resume_path=None, patience=5):
    ...
    with open(log_file, mode) as f:
        ...
        f.write(f"Modèle: {MODEL_NAME} | Époques: {epochs} | LR: {lr}|batch size {batch_size}|patience {patience}\n"
    ...
    for epoch in range(start_epoch, epochs):
        ...
        print(f"Metrics -> Acc: {val_acc:.2f}% | Prec: {val_prec:.2f}% | Rec: {val_rec:.2f}% | F1: {val_f1:.2f}%")
        ...
        debug_full_diagnostic(x_s.to(DEVICE), x_l.to(DEVICE), w.to(DEVICE), model, lbl, CLASSES, epoch + 1,
        ...
        if val_acc > best_acc:
            ...
            if no_imporve >= patience:
                print("early stop")
                with open(log_file, "a") as f:
                    f.write(f"Early stop à l'epoch: {epoch}, pas d'améloration depuis {patience} epochs")
                break
    ...
    with open(log_file, "a") as f:
        f.write(f"=== Entraînement terminé ===\n")
...
if __name__ == "__main__":
    ...
    # Lancement
    train(model, ld_train, ld_val, args.epochs, args.lr, batch_size=args.batch, save_dir=run_save_dir,
          resume_path=args.resume, patience=10)
