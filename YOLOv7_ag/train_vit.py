...
from src.scripts.dataset import DualInputDataset, prepare_dataloaders, find_two_samples, plot_samples_with_weights
from src.scripts.utils.model_loaders import instanciateDualCrossVit
...
def get_data(n_samples=None, batch_size=32, num_workers=6):
    ...
    train_ds = DualInputDataset(
        ...
        classes=CLASSES, paths=("original", "original"), use_yolo_weights=True
    )
...
def validate(model, loader, criterion):
    ...
    with torch.no_grad():
        for img_orig, img_seg, labels, weights in loader:
            img_orig, img_seg, labels = img_orig.to(DEVICE), img_seg.to(DEVICE), labels.to(DEVICE)
