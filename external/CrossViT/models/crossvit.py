...
class VisionTransformer(nn.Module):
    ...
    # NOTE : Added **kwargs for compatibility with later version
    def __init__(
        self,
        img_size=(224, 224),
        patch_size=(8, 16),
        ...
        **kwargs,
        ...
        # number of patches cut in the image for the different branches
        # if patch_size=(8, 16), two branches Small and Large
        num_patches = _compute_num_patches(img_size, patch_size)

        # The branches
        self.num_branches = len(patch_size)

        # List of patch embedding methods for each branches
        self.patch_embed = nn.ModuleList()
        if hybrid_backbone is None:
            # To localize each patch in the image : list of patches for each branch : 1 + num_patches[i] because of the cls token
            ...
            # Loop through branches to adapat the patch embedding
                ...
                self.pos_embed.append(
                    nn.Parameter(
                        data=get_sinusoid_encoding(
                            n_position=1 + num_patches[idx], d_hid=embed_dim[idx]
                        ),
                        requires_grad=False,
                    )
                )
        ...
        for i in range(self.num_branches):
            ...
            x_ = (
                torch.nn.functional.interpolate(
                    x, size=(self.img_size[i], self.img_size[i]), mode="bicubic"
                )
            ...
            # Patch cutting
            # Each branch has its PatchEmbed object, which take image x_ in input
            tmp = self.patch_embed[i](x_)

            cls_tokens = self.cls_token[i].expand(
                B, -1, -1
    ...
    def forward(self, x):
        # [x_small_branch,x_large_branch]
        xs = self.forward_features(x)
        # each branch make prediction
        ce_logits = [self.head[i](x) for i, x in enumerate(xs)]
        # we conclude by the mean of predictions
...
def crossvit_tiny_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            _model_urls["crossvit_tiny_224"], map_location="cpu"
        )
...
def crossvit_base_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            _model_urls["crossvit_base_224"], map_location="cpu"
        )
        model.load_state_dict(state_dict)
    return model
...
def crossvit_9_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            _model_urls["crossvit_9_224"], map_location="cpu"
        )
        model.load_state_dict(state_dict)
    return model
...
def crossvit_15_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            _model_urls["crossvit_15_224"], map_location="cpu"
        )
        model.load_state_dict(state_dict)
    return model
...
def crossvit_18_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            _model_urls["crossvit_18_224"], map_location="cpu"
        )
        model.load_state_dict(state_dict)
    return model
...
def crossvit_9_dagger_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            _model_urls["crossvit_9_dagger_224"], map_location="cpu"
        )
        model.load_state_dict(state_dict)
    return model
...
def crossvit_15_dagger_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            _model_urls["crossvit_15_dagger_224"], map_location="cpu"
        )
        model.load_state_dict(state_dict)
    return model
...
def crossvit_15_dagger_384(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            _model_urls["crossvit_15_dagger_384"], map_location="cpu"
        )
        model.load_state_dict(state_dict)
    return model
...
def crossvit_18_dagger_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            _model_urls["crossvit_18_dagger_224"], map_location="cpu"
        )
        model.load_state_dict(state_dict)
    return model
...
def crossvit_18_dagger_384(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            _model_urls["crossvit_18_dagger_384"], map_location="cpu"
        )
        model.load_state_dict(state_dict)
    return model
