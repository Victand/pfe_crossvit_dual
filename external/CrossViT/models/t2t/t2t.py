...
class T2T(nn.Module):
    ...
    def __init__(
        ...
        self.soft_split0 = nn.Unfold(
            kernel_size=to_2tuple(kernel_size[0][0]),
            stride=to_2tuple(kernel_size[0][1]),
            padding=to_2tuple(kernel_size[0][2]),
        )
        self.soft_split1 = nn.Unfold(
            kernel_size=to_2tuple(kernel_size[1][0]),
            stride=to_2tuple(kernel_size[1][1]),
            padding=to_2tuple(kernel_size[1][2]),
        )
        self.soft_split2 = nn.Unfold(
            kernel_size=to_2tuple(kernel_size[2][0]),
            stride=to_2tuple(kernel_size[2][1]),
        ...
        if tokens_type == "transformer":
            ...
            self.attention1 = Token_transformer(
                dim=in_chans * (kernel_size[0][0] ** 2),
                in_dim=token_dim,
                num_heads=1,
                mlp_ratio=1.0,
            )
            self.attention2 = Token_transformer(
                dim=token_dim * (kernel_size[1][0] ** 2),
                in_dim=token_dim,
                num_heads=1,
                mlp_ratio=1.0,
            )
