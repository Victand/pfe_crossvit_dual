...
from external.CrossViT.datasets import build_dataset
from external.CrossViT.engine import train_one_epoch, evaluate
from external.CrossViT.samplers import RASampler
import external.CrossViT.utils as utils
...
def get_args_parser():
    ...
    parser.add_argument(
        "--pretrained", action="store_true", help="load imagenet1k pretrained model"
    )
    ...
    parser.add_argument(
        "--crop-ratio", type=float, default=256 / 224, help="crop ratio for evaluation"
    )
    ...
    parser.add_argument(
        "--data-path",
        default=os.path.join(os.path.expanduser("~"), "datasets/image_cls/imagenet1k/"),
        type=str,
        help="dataset path",
    )
    parser.add_argument(
...
def main(args):
    ...
    print(
        f"Scaled learning rate (batch size: {args.batch_size * utils.get_world_size()}): {linear_scaled_lr}"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "CrossViT training and evaluation script", parents=[get_args_parser()]
    )
