...
from YOLOv7_ag.utils.plots import color_list, plot_one_box
from YOLOv7_ag.utils.torch_utils import time_synchronized
    ...
    def forward(self, imgs, size=640, augment=False, profile=False, visualize=False):
        ...
        if isinstance(imgs, torch.Tensor):  # torch
            with amp.autocast(enabled=p.device.type != "cpu"):
                return self.model(
                    imgs.to(p.device).type_as(p), augment, profile, visualize
                )  # inference
...
class gatingMechanism(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.conv(
            y
        )  # .squeeze(-1).transpose(-1, -2)).transpose(-1, -2)#.unsqueeze(-1)
        y = self.sigmoid(y)
