...
class DualInputDataset(Dataset):
    ...
    ):  # <-- NOUVEAU: paramètre use_yolo_weights
        """
    ...
    def load_samples(self):
        ...
        for label_int, _class in enumerate(self.classes):
            original_path, segmented_path = (
                self.data_dir / phase / self.paths[0] / _class,
                self.data_dir / phase / self.paths[1] / _class,
            )

            if not (original_path.exists() and segmented_path.exists()):
                raise FileNotFoundError(
                    f"{original_path} or {segmented_path} folder does not exist."
                )
    ...
    def __getitem__(self, idx):
        ...
        original_image, segmented_image = self.synchronized_transform(
        ...
        if self.use_yolo_weights:
            ...
            if weight_p.exists():
                ...
                print(
                    f"⚠️ Poids YOLO introuvables pour {original_image_p.name}, utilisation de poids neutres."
                )
            ...
            weights = self.patches_weights(
                segmented_image, self.patch_size[0], self.weight_function
            )
