...
def dirStructAsTorchvisionLayout(
    labels: DataFrame,
    data_root_path: Path,
    ...
    for code in keys:
        ...
        original_dest_path = (
            data_root_path / f"{current_mode}/{branches[0]}/{current_class}"
        )
        segmented_dest_path = (
            data_root_path / f"{current_mode}/{branches[1]}/{current_class}"
        )
    ...
    print("-" * 20)
    print(
        f"Train split : {train_split * 100}%    No epines/epines ratio : {int(class1_ratio * 100)}/{100 - int(class1_ratio * 100)}"
    )
